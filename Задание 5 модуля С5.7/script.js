let localStorageKey = localStorage.getItem('');
  ...
  return fetch(url)
    .then((response) => {
      ...
      // надо отдать в следующий then
		...
		.then((jsonData) => {
      // Объект результата jsonData в формате JSON
    
      //запишем полученные данные в localStorage
...
btnNode.addEventListener('click', async () => {
  ...
  const url = `https://picsum.photos/v2/list?page=${numberPage}&limit=${limit}`;
	...
	/* Если введенные данные прошли проверку, прежде всего, проверим 
  нет ли такого url в localStorage. Если есть, то сразу вызовем функцию displayResult, 
  c данными из localStorage	*/

  localStorageKey = localStorage.getItem(url);
  if (localStorageKey) {
    const apiData = JSON.parse(localStorageKey);
    await displayResult(apiData);
    console.log('Ссылки на картинки получены из localStorage');
  } else {
